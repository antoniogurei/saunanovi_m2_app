...
info "Run database upgrade."
${PHP_BIN} "${DIR_MAGE}/bin/magento" setup:upgrade

info "Perform maintenance tasks according to deployment mode (${MODE})."
...
if test -z "${LOCAL_OWNER}" || test -z "${LOCAL_GROUP}" || test -z "${DIR_MAGE}"; then
  ...
  chown -R "${LOCAL_OWNER}":"${LOCAL_GROUP}" "${DIR_LINK_LOG}/"
  chmod 770 ${DIR_LINK_LOG}/
  chmod 660 ${DIR_LINK_LOG}/*.log
fi
